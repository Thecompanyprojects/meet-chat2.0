...
@interface KKLDragCardContainer()<UIGestureRecognizerDelegate>
/** YFLDragCardView实例的集合 **/
@property (nonatomic,strong) NSMutableArray <KKLDragCardView *> *cards;
/** 滑动方向 **/
...
@implementation KKLDragCardContainer
...
- (KKLDragConfigure*)setDefaultsConfigures
{
    KKLDragConfigure *configure = [[KKLDragConfigure alloc]init];
...
- (void)initDataConfigure:(KKLDragConfigure*)configure
{
    [self resetInitData];
...
- (void)addSubViews
{
    ...
    if (self.loadedIndex <  sum)
    {
        ...
        {
            KKLDragCardView *cardView = [self.dataSource container:self viewForRowsAtIndex:self.loadedIndex];
...
- (void)resetLayoutSubviews
{
    ...
    [UIView animateWithDuration:0.5f delay:0.0f usingSpringWithDamping:0.6 initialSpringVelocity:0.6 options:UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        for (int i = 0; i < self.cards.count; i++){
            KKLDragCardView *cardView = [self.cards objectAtIndex:i];
...
- (void)recordFrame:(KKLDragCardView *)cardView
{
...
- (void)moveIngStatusChange:(float)scale
{
    ...
    if (!self.isMoveIng) {
        ...
        for (int index = 1; index < self.cards.count ; index++) {
            KKLDragCardView *cardView = (KKLDragCardView *)self.cards[index];
            switch (index) {
                case 1:
                {
...
- (void)panGesturemMoveFinishOrCancle:(KKLDragCardView*)cardView direction:(ContainerDragDirection)direction scale:(float)scale isDisappear:(BOOL)isDisappear
{
    if (!isDisappear) {
        ...
        if (self.isMoveIng && self.cards.count > self.configure.visableCount) {
            ...
            KKLDragCardView *lastView = (KKLDragCardView *)self.cards.lastObject;
...
- (void)reloadData
{
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfRowsInYFLDragCardContainer:)] && [self.dataSource respondsToSelector:@selector(container:viewForRowsAtIndex:)]) {
        ...
        //刷新关键代码
        for (KKLDragCardView * view in self.subviews) {
            if([view isKindOfClass:[KKLDragCardView class]]){
...
//    KKLDragCardView *currentShowCardView = self.cards.firstObject;
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (self.cards.count > 3) {
            KKLDragCardView * tCardView = [self.cards objectAtIndex:3];
...
- (void)removeCardViewForDirection:(ContainerDragDirection)direction
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        //卡片滑动结束的代理(可用户发送数据请求)
        if ([self.delegate respondsToSelector:@selector(container:dragDisappearForDirection:forCardView:)]) {
            
            [self.delegate container:self dragDisappearForDirection:direction forCardView:currentShowCardView];
        }
...
- (void)handlePanGesture:(UIPanGestureRecognizer*)pan
{
    ...
    if ([self.delegate respondsToSelector:@selector(container:canDragForCardView:)]) {
        canEdit = [self.delegate container:self canDragForCardView:(KKLDragCardView*)pan.view];
    }
    ...
    if (canEdit) {
        ...
        if (pan.state == UIGestureRecognizerStateBegan){
            ...
            KKLDragCardView *cardView = (KKLDragCardView *)pan.view;
            ...
            [self panGesturemMoveFinishOrCancle:(KKLDragCardView*)pan.view direction:self.direction scale:moveX/moveY isDisappear:fabs(horizionSliderRate)>boundaryRation];
            
            if (self.direction == ContainerDragRight) {
                 [self logManagerLiketype:@{@"type":@"0"}];
            }else if (self.direction == ContainerDragLeft){
                [self logManagerDisliketype:@{@"type":@"0"}];
            }
        }
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (useModel.userphotos.count==0) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"UPLOADIMAGESHOW" object:nil];
    }
    ...
    if (![KKUserModel sharedUserModel].isVip) {
        if (![[KKShowsubscribeModel sharedShowsubModel] likedCanShow]) {
            if ([[KKShowsubscribeModel sharedShowsubModel] isNewSub]) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"VPSHOWVIDEO" object:nil];
                
            }
            else
            {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"VPSHOW" object:nil];
