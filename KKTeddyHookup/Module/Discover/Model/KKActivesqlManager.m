...
@interface KKActivesqlManager()
@property(nonatomic,strong) FMDatabase *db;
@property(nonatomic,strong) NSString * dbPath;
@property(nonatomic,strong) NSMutableArray *dataArray;
@end

@implementation KKActivesqlManager

+ (instancetype)sharedClient
{
    static KKActivesqlManager *_sqlManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sqlManager = [[KKActivesqlManager alloc]init];
    });
    return _sqlManager;
}
...
-(void)creatTable
{
    ...
    if ([db open]) {
        //4.创表
        BOOL result=[db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_activeData (id INTEGER PRIMARY KEY , name TEXT NOT NULL, age TEXT , signature TEXT , horoscope TEXT , city TEXT , single TEXT , photo VARCHAR , photopreview VARCHAR , video VARCHAR , videopreview VARCHAR , issayHi INTEGER);"];
        if (result){
            NSLog(@"创表成功");
        }else{
            NSLog(@"创表失败");
...
-(void)instrtmymodel
{
    ...
    @try {
        for (int i = 0; i<self.dataSource.count; i++) {
            ...
            NSString *sql = @"INSERT INTO t_activeData (id,name,age,signature,horoscope,city,single,photo,photopreview,video,videopreview,issayHi) VALUES (?,?,?,?,?,?,?,?,?,?,?,?);";
...
-(void)deletefromData
{
    ...
    BOOL result = [self.db executeUpdate:@"DELETE FROM t_activeData"];
    if (result) {
...
-(NSMutableArray *)loaddata
{
    ...
    FMResultSet *resultSet = [dataBase executeQuery:@"select * from t_activeData"];
...
- (void)updateDataWithuserId:(NSString *)newId {
    ...
    FMDatabase *dataBase = [FMDatabase databaseWithPath:dbPath];
    if ([dataBase open]) {
        NSString *sql = @"UPDATE t_activeData SET issayHi = ? WHERE  id = ?";
        NSInteger idinter = [newId integerValue];
