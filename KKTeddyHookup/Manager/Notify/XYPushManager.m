...
#import "XYPushManager_Extension.h"
#import <FCUUID/FCUUID.h>
//#import "XTNetworkHelper.h"
@interface XYPushManager()<UNUserNotificationCenterDelegate>

@end

@implementation XYPushManager
//MARK:- 删除所有通知
...
+ (void)registerDeviceToken:(NSData *)deviceToken{
    ...
    NSDictionary *params = @{@"mid":uuid,
                            @"token":deviceString};
    
//    AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc]initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
//    NSURLSessionDataTask *task = [manager POST:[NSString stringWithFormat:@"%@%@",ServerIP,uploadDeviceToken] parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
//        NSLog(@"上报Device成功:\n%@", responseObject)
...
//    [task resume];
    ...
    NSDictionary *dic = [XYNetworkTools handleParameter:@{@"data":params}];
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [session POST:[NSString stringWithFormat:@"%@%@",ServerIP,uploadDeviceToken] parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
         NSLog(@"上报Device成功:\n%@", responseObject)
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
          NSLog(@"上报Device失败:\n%@", error)
    }];
...
static XYPushManager *shared = nil;
+ (XYPushManager *)shared
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared = [[XYPushManager alloc] init];
    });
    return shared;
}
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler API_AVAILABLE(ios(10.0)){
    ...
    if([notification.request.trigger isKindOfClass:[UNPushNotificationTrigger class]]) {
        [XYPushManager handleRemoteNotification:userInfo];
    }
