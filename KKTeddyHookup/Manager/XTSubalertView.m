...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.alertView.frame = CGRectMake(MARGIN, 160*KHEIGHT, WIDTH-60, 282);
...
-(void)chooseVideo
{
    ...
    switch (self.alertType) {
        case Alertadlike:
            [XTincentiveManager sharedClient].type = Subactiveadlike;
            break;
        case Alertadshake:
            [XTincentiveManager sharedClient].type = Subactiveadshake;
            break;
        case Alertadalive:
            [XTincentiveManager sharedClient].type = Subactiveadalive;
            break;
        case Alertadsayhi:
            [XTincentiveManager sharedClient].type = Subactiveadsayhi;
            break;
        case Alertadbottle:
            [XTincentiveManager sharedClient].type = Subactiveadbottle;
            break;
        case Alertadgetbottle:
            [XTincentiveManager sharedClient].type = SubactiveadgetBottle;
            break;
        case AlertwithRobit:
            [XTincentiveManager sharedClient].type = Subdiscoverunlook;
    ...
    [[XTincentiveManager sharedClient] loadrewardVideo];
    [[XTincentiveManager sharedClient] withSurevideoClick:^(NSString * _Nonnull string) {
        if (self.sureClick) {
            self.sureClick([NSString new]);
        }
    }];
    [[XTincentiveManager sharedClient] withReturnvideoClick:^(NSString * _Nonnull string) {
